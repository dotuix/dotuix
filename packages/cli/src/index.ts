...
function offlineCheck(
  ...
  for (const [path, data] of Object.entries(files)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const num = i + 1;
      if (FONT_RE.test(ln))
        issues.push({ file: path, line: num, message: `Google Fonts import — will fail offline` });
      else if (CDN_RE.test(ln))
        issues.push({ file: path, line: num, message: `CDN dependency — will fail offline` });
      else if (EXTERNAL_RE.test(ln) || FETCH_RE.test(ln) || WS_RE.test(ln))
        issues.push({ file: path, line: num, message: `External URL — will fail offline: ${ln.trim().slice(0, 80)}` });
    }
  }
...
async function cmdPack(args: string[]) {
  ...
  if (!dir) { console.error(c.red("✗") + " Usage: dotuix pack <dir> [-o out.uix]"); process.exit(1); }
...
async function cmdUnpack(args: string[]) {
  ...
  if (!file) { console.error(c.red("✗") + " Usage: dotuix unpack <file.uix> [-o outDir]"); process.exit(1); }
...
async function cmdValidate(args: string[]) {
  const file = pos(args)[0];
  if (!file) { console.error(c.red("✗") + " Usage: dotuix validate <file.uix>"); process.exit(1); }
  ...
  } catch { /* structural errors captured by validate() */ }
...
async function cmdInfo(args: string[]) {
  const file = pos(args)[0];
  if (!file) { console.error(c.red("✗") + " Usage: dotuix info <file.uix>"); process.exit(1); }
  ...
  if (manifest.expires) {
    ...
    console.log(`  ${c.muted("expires:")}     ${manifest.expires}` + (expired ? c.red(" (expired)") : c.green(" (active)")));
  }
  if (manifest.author) {
    const a = manifest.author;
    const who = [a.name, a.email ? `<${a.email}>` : "", a.url ?? ""].filter(Boolean).join(" ");
...
async function cmdExport(args: string[]) {
  ...
  if (!file || !type) {
    console.error(c.red("✗") + " Usage: dotuix export <file.uix> --type <type> [--format json|csv] [-o file]");
    process.exit(1);
  }
  if (format !== "json" && format !== "csv") {
    console.error(c.red("✗") + " --format must be json or csv"); process.exit(1);
  }
  ...
  const rows = records.map((r: UIXRecord) => {
    ...
    try { body = JSON.parse(r.body as string); } catch { body = { raw: r.body }; }
    return { id: r.id, type: r.type, created_at: r.created_at, updated_at: r.updated_at, ...body };
  });
  ...
  if (format === "json") {
    ...
    content = [keys.join(","), ...rows.map(r => keys.map(k => esc((r as Record<string, unknown>)[k])).join(","))].join("\n");
  }
...
async function cmdInit(args: string[]) {
  ...
  if (existsSync(dir)) {
    console.error(c.red("✗") + ` Already exists: ${dir}`); process.exit(1);
  }
...
function printHelp() {
  ...
  ${c.bold("Commands:")}
    ...
    ${c.cyan("init")}     [name]                              Scaffold a new project
    ${c.cyan("export")}   <file.uix> --type <t>               Export state records
...
async function main() {
  ...
  if (!cmd || flag([cmd], "-h", "--help", "help")) { printHelp(); return; }
  switch (cmd) {
    case "pack":     await cmdPack(rest);     break;
    case "unpack":   await cmdUnpack(rest);   break;
